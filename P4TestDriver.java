 ...
 * 
 * <br>Revisions:
 *    <br>2.1 Dave Parkinson - Added test driver line output, Treasure location test
...
public class P4TestDriver
{
   ...
   {  
      ...
      // test reading a good maze, check that Treasures are located properly
      System.out.println("Start");
      maze = new Maze();
      maze.readMazeFromFile("mazeGood.txt");
      // Should only be two treasures, located at 2,2 and 4,4
      //System.out.println(maze.getSquare(2,2).treasure());
      //System.out.println(maze.getSquare(4,4).treasure());
      pass &= test(maze.getSquare(2,2).treasure() != null, test++); // test 26
      pass &= test(maze.getSquare(4,4).treasure() != null, test++); // test 27
      int treasCount = 0;
      for (int i = 0; i < maze.rows(); i++){
         for (int j = 0; j < maze.cols(); j++){
            if (maze.getSquare(i,j).treasure() != null){
               treasCount++;
         }}
      }

      pass &= test(treasCount == 2, test++);  // test 28
